...
#define tpush_simple(tok)						\
  tpush(((struct Token){						\
	.ty = tok, .loc = this_loc(self), .start = &(self->contents[self->index]), .token_len = 1}))
#define tpush(tok) vec_push_one(self->stream->tokens, tok)
#define tpush_str(tok, str, len)                                               \
  tpush((struct Token){                                                        \
      .ty = tok, .loc = this_loc(), .start = str, .token_len = len})

static const char utility_keywords[][KEYWORD_MAX] = {
    "_Alignas",  "_Alignof",       "_Atomic",       "_Bool",
    "_Complex",  "_Generic",       "_Imaginary",    "_Pragma",
    "_Noreturn", "_Static_assert", "_Thread_local", "_Typeof",
    "_Vector",   "__cdecl",        "__stdcall",     "__declspec",
};

#define UTILITY_KEYWORDS_LEN (sizeof(utility_keywords) / KEYWORD_MAX)

static const char keywords[][KEYWORD_MAX] = {
    "while",    "break",  "case",    "const",  "continue", "volatile", "void",
    "unsigned", "union",  "default", "do",     "double",   "else",     "if",
    "enum",     "extern", "typedef", "switch", "struct",   "static",   "sizeof",
    "signed",   "float",  "for",     "goto",   "inline",   "int",      "long",
...
void rewind_lexer(struct Lexer *self) {
  self->index--;
  self->col--;
}
...
enum TokenType get_corresponding_keyword_type(char *keyword,
                                              bool *match_found) {
...
struct Token lex_ident(struct Lexer *self) {
  ...
  struct Token t = (struct Token){.loc = this_loc(self),
                                  .start = final_string,
...
void lexer_lex(struct Lexer *self) {
  ...
  do {
	...
	if (isspace(chr)) {
	  ...
	  rewind_lexer(self);
        ...
        default: { 
          // No comment found, just a div sign.
	  rewind_lexer(self);
	  tpush_simple(TOKEN_DIV);
	  break;
        }
        }
      } else { die("unexpected eof"); }
      break;
    }
    case TOKEN_INTEGER:
  ...
  void lexer_dump(struct Lexer * self) {
    ...
    FOR_EACH_VEC(self->stream->tokens, it, {
      printf("Token {\n\t[%s]: \n\t Len %lu;\n\t Line %d;\n\t Column %d;\n\t "
             "Payload: %.*s\n}\n",
             token_str[it.ty], it.token_len, it.loc.line_number,
             it.loc.column_number, (int)it.token_len, it.start);
    });
  }
