...
void *_vec_new(usize elem_size, usize cap) {
  ...
  if (!info) {
    die("failed to allocate new vector");
  }
...
void *_vec_prep_push(void *self, usize count) {
  struct VecInfo *info = GET_INFO(self);
  if ((info->cap * info->elem_size) <
      (info->len * info->elem_size) + (count * info->elem_size)) {
    struct VecInfo *new = vec_grow(self, ((info->len * info->elem_size) +
                                          (info->len * info->elem_size) / 2 +
                                          (count * info->elem_size)));
    return &new->data[0];
  }
  return self;
}
...
usize vec_len(void *self) { return GET_INFO(self)->len; }