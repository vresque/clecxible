...
const ichar *G_USAGE =
    "Usage: clecxible [file(s)] [option(s)]\n"
    ...
    "\t--platform <windows|unix> Specifies the platform to produce the binary "
    "for\n"
    "\t--format <coff|elf> Specifies the format of the binary\n"
    "\t-O --optimization <0,1,2,3,size> Specifies the optimization level, "
    "where 0 means no optimization at all and 3 demands aggressive "
    "optimization. Size tries to shrink the binary size\n";
void usage() { printf("%s\n", G_USAGE); }
